...
export default class GardenScene extends Phaser.Scene {
  ...
  create() {
    ...
    if (wallLayer) try { wallLayer.setCollisionByExclusion([-1]); } catch (e) { wallLayer.setCollisionByProperty({ collides: true }); }
    if (treeLayer) try { treeLayer.setCollisionByExclusion([-1]); } catch (e) { treeLayer.setCollisionByProperty({ collides: true }); }

    // Always spawn player near exit object layer
    ...
    if (exitObjLayer) {
      ...
      if (exitObj) {
        // Spawn near the exit object (offset by ~50 pixels)
        spawnX = exitObj.x - 50;
        spawnY = exitObj.y - 50;
      }
    }

    this.player = this.physics.add.sprite(spawnX, spawnY, "player", 0).setScale(0.18);
    ...
    this.npcA = this.physics.add.sprite(benchX - 16, benchY, "femaleAdventurer", "idle").setScale(npcScale);
    this.npcB = this.physics.add.sprite(benchX + 16, benchY, "femalePerson", "idle").setScale(npcScale);
    ...
    this.gardener = this.physics.add.sprite(lakeX, lakeY, "malePerson", "idle").setScale(npcScale);
    this.gardener.setDepth(140);
    this.gardener.body.setCollideWorldBounds(true);
    if (this.gardener.body) {
      this.gardener.setSize(this.gardener.width * 0.4, this.gardener.height * 0.2);
      this.gardener.setOffset(this.gardener.width * 0.3, this.gardener.height * 0.8);
    }
    ...
    ['femaleAdventurer', 'femalePerson', 'malePerson'].forEach((key) => {
      if (!this.anims.exists(`${key}-walk`)) {
        this.anims.create({
          key: `${key}-walk`,
          frames: this.anims.generateFrameNames(key, { start: 0, end: 7, prefix: 'walk', zeroPad: 0 }),
          frameRate: 8,
          repeat: -1,
        });
      }
    ...
    const makeSpeechBubble = (x, y, msg, index = 0) => {
      const txt = this.add.text(0, 0, msg, { fontSize: '14px', color: '#000000', align: 'center', wordWrap: { width: 100 } }).setOrigin(0.5);
      const paddingX = 5;
      const paddingY = 2;
    ...
    this.physics.add.overlap(this.player, benchZone, () => {
      ...
      const arriveCheck = this.time.addEvent({
        ...
        loop: true,
        callback: () => {
          const dist = Phaser.Math.Distance.Between(this.gardener.x, this.gardener.y, targetX, targetY);
          if (dist < 8) {
    ...
    if (exitObj) {
      console.log("Exit object found at:", exitObj.x, exitObj.y, "Size:", exitObj.width, exitObj.height);
      ...
      this.physics.add.overlap(this.player, this.exitZone, () => {
        console.log("Player overlapping exit zone - transitioning to GameScene");
  ...
  showQuiz() {
    ...
    this.quizText = this.add.text(boxX + 18, boxY + 18, "Two kids are fighting over who broke the bench, what will you do:", {
      fontSize: '11px',
      fill: '#000',
      wordWrap: { width: boxWidth - 36, useAdvancedWrap: true },
      lineSpacing: 2
    }).setScrollFactor(0).setDepth(1000).setVisible(true);
    ...
    options.forEach((option, i) => {
      const opt = this.add.text(boxX + 22, optionY, option, {
        fontSize: '10px', 
        fill: '#000',
  ...
  handleQuizSelection(selectedKey) {
    ...
    resultBox.fillRoundedRect(cam.width/2 - 114, cam.height/2 - 25, 228, 50, 8);
    resultBox.lineStyle(2, 0x000000, 1);
    resultBox.strokeRoundedRect(cam.width/2 - 114, cam.height/2 - 25, 228, 50, 8);
  ...
  createAnimations() {
    ...
    if (!g.exists('walk-down')) {
      g.create({ key: 'walk-down', frames: g.generateFrameNames('player', { start: 22, end: 23 }), frameRate: 8, repeat: -1 });
    }
    if (!g.exists('walk-left')) {
      g.create({ key: 'walk-left', frames: g.generateFrameNames('player', { start: 16, end: 18 }), frameRate: 8, repeat: -1 });
    }
    if (!g.exists('walk-right')) {
      g.create({ key: 'walk-right', frames: g.generateFrameNames('player', { start: 19, end: 21 }), frameRate: 8, repeat: -1 });
    }
    if (!g.exists('walk-up')) {
      g.create({ key: 'walk-up', frames: g.generateFrameNames('player', { start: 22, end: 23 }), frameRate: 8, repeat: -1 });
    }
  }
}